...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let eva_url =
        std::env::var("EVA_ICS_URL").unwrap_or_else(|_| "http://localhost:7727".to_string());
    let eva_api_key =
        std::env::var("EVA_ICS_API_KEY").unwrap_or_else(|_| "default-key".to_string());
    ...
    info!(
        "Connected to Zenoh at {} and EVA-ICS at {}",
        zenoh_endpoint, eva_url
    );
